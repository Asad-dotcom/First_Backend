...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Somewent Wrong While Gerenating Access and Refresh Tokken"
    );
  }
};
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      throw new ApiError(401, "Unvalid refreshToken");
    }
...
const getCurrentUser = asyncHandler(async (req, res) => {
  return res
    .status(200)
    .json(new ApiResponse(200, req.user, "Current User Feacth Sucessfully!"));
});
...
const updateAccountDetail = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(200, user, "User details Updated Sucessfully");
});
...
const updateUserAvtar = asyncHandler(async (req, res) => {
  ...
  const avatar = await uploadOncloudinary(avatarLocalPath);
  if (!avatar.url) {
    throw new ApiError(400, "Avatar is not Upload on clouldary");
  }
...
const updateCoverImage = asyncHandler(async (req, res) => {
    ...
    const coverImage =  await uploadOncloudinary(coverImageLocalPath)
if(!coverImage?.url){
    throw new ApiError(400, "CoverImage is Not uploaded on clouldary")
}
const user =  await User.findByIdAndUpdate(
req.user?._id,
{
...
return res.status(200).json(
     new ApiResponse(
                200,
                user, // Return user directly, not wrapped in object
